...
public sealed class AcademicServiceClient
{
    ...
    public AcademicServiceClient(HttpClient http, IConfiguration config)
    {
        _http = http;
        var baseUrl = config["Services:AcademicService"]
            ?? throw new InvalidOperationException("Services:AcademicService no configurado en appsettings.json");
